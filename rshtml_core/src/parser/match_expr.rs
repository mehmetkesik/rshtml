    ...
    fn parse(parser: &mut RsHtmlParser, pair: Pair<Rule>) -> Result<Node, Box<Error<Rule>>> {
        ...
        for match_expr_arm in match_expr_arms {
            ...
            let node_arm_value = match match_expr_arm_value.as_rule() {
                ...
                Rule::continue_directive => vec![Node::ContinueDirective(match_expr_arm_value_position)],
            ...
            nodes.push(((match_expr_arm_head.as_str().to_string(), match_expr_arm_head_position), node_arm_value));
        }

        Ok(Node::MatchExpr(
            (match_expr_head.as_str().to_string(), match_expr_head_position),
            nodes,
            position,
        ))
    }
